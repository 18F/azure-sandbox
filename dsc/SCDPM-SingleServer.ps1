...
Configuration DPM
{
    ...
    Import-DscResource -ModuleName @{ModuleName="xDismFeature";ModuleVersion="1.1.0.0"}
    Import-DscResource -ModuleName @{ModuleName="xSqlServer"; ModuleVersion="2.0.0.0"}
    Import-DscResource -ModuleName @{ModuleName="xSCDPM"; ModuleVersion="1.2.0.0"}
...
foreach($Node in $ConfigurationData.AllNodes)
{
    if($Node.NodeName -ne "*")
    {
        Write-Warning -message "Here is where we skipped the Copy-Item we'll need to do next time"
###### THIS:
#        $cs = New-PSSession -ConnectionUri $uri -Credential $credential
#        Copy-Item -Path 'C:\Program Files\WindowsPowerShell\Modules' -Destination 'C:\Program Files\WindowsPowerShell\Modules' -R -ToSession $cs -Verbose
##### not this:
